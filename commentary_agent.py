...
def select_pdfs_and_keywords(question: str, pdfs: dict) -> tuple[dict, list[str]]:
    """Single Claude call: identify relevant PDFs AND generate search keywords."""
    ...
    response = client.messages.create(
        ...
        messages=[{
            ...
            "content": (
                ...
                "FILES: list only the filenames most relevant to this question.\n"
                "KEYWORDS: verse references AND synonyms/related terms "
                "(e.g. for 'What about forgiveness?' → forgiveness, forgive, pardon, mercy, repentance)."
            )
        }]
    )
    ...
    filtered = {
        ...
        if any(r in meta["filename"] or meta["filename"] in r for r in relevant_names)
    }
    return (filtered if filtered else pdfs), keywords
...
def cmd_ask(question: str):
    ...
    # Step 1: single call — identify relevant PDFs and generate search keywords
    print(f"Finding relevant commentaries from {len(pdfs)} PDFs...", flush=True)
    relevant, keywords = select_pdfs_and_keywords(question, pdfs)
    print(f"Scanning {len(relevant)} PDF(s) for matching pages...", flush=True)

    # Step 2: download each PDF from Drive and extract matching pages
